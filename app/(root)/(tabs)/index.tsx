      ...
      <SafeAreaView className="flex-1">
        ...
        <ScrollView className="px-3 flex-1 mb-14">
          ...
          <View className="flex-row flex-wrap">
            {popularPlants
              .slice(0, showAllPopular ? popularPlants.length : 4)
              .map((item) => renderPlantCard(item))}
          </View>

          {/* Cây tương tự */}
          <View className="flex flex-row justify-between items-center py-2 mt-4">
            <Text className="text-2xl font-bold text-black">
              Tương tự cây của bạn
            </Text>
            <TouchableOpacity
              onPress={() => setShowAllSimilar(!showAllSimilar)}
            >
              <Text className="text-[rgb(60,193,142)]">
          ...
          <View className="flex-row flex-wrap">
            {similarPlants
              .slice(0, showAllSimilar ? similarPlants.length : 4)
              .map((item) => renderPlantCard(item))}
          </View>
        </ScrollView>
      </SafeAreaView>
